...
const SAMPLE = {
  ...
  description: "درخواست نمونه برای بررسی عملکرد موتور تطابق.",
...
export default function BuyerRequestFormPage() {
  ...
  const submit = async (e) => {
    ...
    setError(null);
    if (!isAuthenticated) {
      setError("برای ثبت درخواست ابتدا وارد شوید.");
      return;
    }
    setSaving(true);
    try {
      ...
      setError(apiError(err, "خطا در ثبت درخواست"));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div>
      <header className="page-header">
        <div>
          <h1 className="title">ثبت درخواست خرید</h1>
          <p className="lead">معیارهای مورد نظر خریدار را وارد کنید.</p>
        </div>
      ...
      {!isAuthenticated && (
        <div className="alert alert-caution">
          برای ثبت درخواست باید وارد شوید. <Link to="/login">ورود</Link> یا از نوار بالای
          صفحه حساب خریدار را انتخاب کنید.
        </div>
      )}

      <form className="card" onSubmit={submit}>
        <fieldset className="fieldset">
          <legend className="fieldset-legend">نوع ملک</legend>
        ...
        <fieldset className="fieldset">
          <legend className="fieldset-legend">معیارها</legend>
          <div className="grid-3">
        ...
        <div className="form-actions">
          ...
          <Link to="/requests" className="btn btn-quiet">انصراف</Link>
        </div>
      </form>
    </div>
  );
